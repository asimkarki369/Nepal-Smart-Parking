// src/screens/ZoneDetailScreen.tsx
...
export default function ZoneDetailScreen() {
  ...
  const zone = mockZones.find((z: any) => z.code === route.params.zoneCode) ?? mockZones[0];
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.card}>
          <View style={styles.zoneHeaderRow}>
            ...
            <View style={[styles.tag, isFull ? styles.tagRed : zone.occupancyPercent >= 70 ? styles.tagAmber : styles.tagGreen]}>
              <Text style={[styles.tagText, isFull ? { color: Colors.red } : zone.occupancyPercent >= 70 ? { color: Colors.accent } : { color: Colors.green }]}>
          ...
          <View style={styles.toggleRow}>
            {(['4w', '2w'] as const).map(vt => (
              <TouchableOpacity key={vt} style={[styles.toggleBtn, vehicleType === vt && styles.toggleActive]}
                onPress={() => setVehicleType(vt)}>
                <Icon name={vt === '4w' ? 'car' : 'motorbike'} size={18}
          ...
          {/* Cost breakdown */}
          <View style={styles.costRow}><Text style={styles.costLabel}>Parking fee</Text><Text style={styles.costValue}>Rs {fee}</Text></View>
          <View style={styles.costRow}><Text style={styles.costLabel}>Service fee (10%)</Text><Text style={styles.costValue}>Rs {serviceFee}</Text></View>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              if (isFull) { Alert.alert('Zone Full', 'This zone is currently full. Please choose another zone.'); return; }
